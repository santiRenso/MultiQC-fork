...
const svgContainer = document.querySelector(el).closest(".svg-container");
  ...
  const resizeObserverBuild = new ResizeObserver(debounce((entries) => {
    for (const entry of entries) {
      if (entry.borderBoxSize && entry.borderBoxSize[0].inlineSize!==chartSize) {
        chartSize = entry.borderBoxSize[0].inlineSize;
